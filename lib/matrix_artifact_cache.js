...
function readRegularFileNoFollow(file, encoding = null) {
  const noFollow = fs.constants.O_NOFOLLOW || 0;
  const fd = fs.openSync(file, fs.constants.O_RDONLY | noFollow);
  try {
    const stat = fs.fstatSync(fd);
    if (!stat.isFile()) throw new Error('MATRIX_CACHE_FILE_INVALID');
...
function loadMatrixArtifactCache({ dataDir, requestId, num, fingerprint, recipient, now = Date.now() }) {
  ...
  try {
    ...
    const manifest = JSON.parse(readRegularFileNoFollow(manifestPath, 'utf8'));
    ...
    for (const item of manifest.documents) {
      ...
      const buffer = readRegularFileNoFollow(file);
...
function purgeExpiredMatrixArtifactCaches(dataDir, now = Date.now()) {
  ...
  try {
    for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
      // Une écriture est synchrone et atomique dans ce processus; un dossier
      // .partial-* visible ici provient donc d'un arrêt brutal précédent. Le
      // motif strict empêche toute suppression en dehors du cache Matrix.
      if (/^\.partial-mx[a-f0-9]{16}-[a-f0-9]{8}$/.test(entry.name)) {
        const partialPath = path.join(root, entry.name);
        try {

...
const CENTRIS_PUBLIC_HOSTS = new Set(['www.centris.ca']);
const PIPEDRIVE_API_HOSTS = new Set(['api.pipedrive.com']);
const BREVO_API_HOSTS = new Set(['api.brevo.com']);
...
function validatePipedriveApiUrl(rawUrl) {
  const parsed = parseStrictHttpsUrl(rawUrl, PIPEDRIVE_API_HOSTS);
  if (!/^\/(?:v1|api\/v2)\//.test(parsed.pathname)) throw new Error('CHEMIN_PIPEDRIVE_INTERDIT');
  return parsed;
}
function validateBrevoApiUrl(rawUrl) {
  const parsed = parseStrictHttpsUrl(rawUrl, BREVO_API_HOSTS);
  if (!/^\/v3\/emailCampaigns\/\d+(?:\/(?:status|sendTest|sendNow))?$/.test(parsed.pathname)) {
    throw new Error('CHEMIN_BREVO_INTERDIT');
  }
  if (parsed.search || parsed.hash) throw new Error('PARAMETRES_BREVO_INTERDITS');
  return parsed;
}
function brevoCampaignUrl(campaignId, action = '') {
  const rawId = String(campaignId ?? '').trim();
  if (!/^[1-9]\d{0,14}$/.test(rawId)) throw new Error('ID_CAMPAGNE_BREVO_INVALIDE');
  const normalizedAction = String(action || '');
  if (normalizedAction && !['status', 'sendTest', 'sendNow'].includes(normalizedAction)) {
    throw new Error('ACTION_CAMPAGNE_BREVO_INVALIDE');
  }
  const suffix = normalizedAction ? `/${normalizedAction}` : '';
  return validateBrevoApiUrl(`https://api.brevo.com/v3/emailCampaigns/${rawId}${suffix}`);
}
function validateCentrisPublicListingUrl(rawUrl) {
  const parsed = parseStrictHttpsUrl(rawUrl, CENTRIS_PUBLIC_HOSTS);
  if (!/^\/fr\/properties~a-vendre\/\d{6,10}$/.test(parsed.pathname)) {
    throw new Error('CHEMIN_CENTRIS_PUBLIC_INTERDIT');
  }
  if (parsed.search || parsed.hash) throw new Error('PARAMETRES_CENTRIS_PUBLIC_INTERDITS');
  return parsed;
}
function centrisPublicListingUrl(centrisNumber) {
  const rawNumber = String(centrisNumber ?? '').trim();
  if (!/^\d{6,10}$/.test(rawNumber)) throw new Error('NUMERO_CENTRIS_INVALIDE');
  return validateCentrisPublicListingUrl(`https://www.centris.ca/fr/properties~a-vendre/${rawNumber}`);
}
...
module.exports = {
  ...
  validateBrevoApiUrl,
  brevoCampaignUrl,
  validateCentrisPublicListingUrl,
  centrisPublicListingUrl,
  secretTestTarget,

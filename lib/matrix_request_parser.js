...
function parseDirectMatrixRequest(value) {
  ...
  // Forme ultra-courte conservée: « 19465925 client@example.com message ».
  let match = text.match(new RegExp(`^#?(${CENTRIS_RE})\\s+(${EMAIL_RE})(?:\\s+(.+))?$`, 'i'));
  if (!match) {
...
function looksLikeMatrixSendWithoutEmail(value) {
  const text = cleanMessage(value);
  if (!/^envoie(?:r)?\b/i.test(text) || new RegExp(EMAIL_RE, 'i').test(text)) return false;

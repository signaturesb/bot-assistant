...
const NATURAL_SEND_PREFIX =
  `(?:` +
    `envoie(?:s)?(?:-moi|-lui)?(?:\\s+(?:lui|y))?|` +
    `envoyer(?:\\s+(?:lui|y))?|pr[ée]pare|pr[ée]parer|` +
    `(?:peux[-\\s]tu|peut[-\\s]tu|tu\\s+peux|pourrais[-\\s]tu)\\s+(?:(?:lui|y)\\s+)?envoyer|` +
    `je\\s+veux\\s+que\\s+tu\\s+(?:(?:lui|y)\\s+)?envoies?|` +
    `(?:fais|fait)(?:-lui|\\s+lui)?\\s+parvenir` +
  `)`;

function findEmails(text) {
  return String(text || '').match(new RegExp(EMAIL_RE, 'gi')) || [];
}

function textWithoutEmails(text) {
  return String(text || '').replace(new RegExp(EMAIL_RE, 'gi'), ' ');
}

function cleanMessage(value) {
  return String(value || '').replace(/[\r\0]+/g, ' ').replace(/\s+/g, ' ').trim().replace(/^[«»“”"]+|[«»“”"]+$/g, '').trim();
}
...
function parseDirectMatrixRequest(value) {
  ...
  let match = text.match(new RegExp(
    `^#?(${CENTRIS_RE})\\s+` +
    `(?:(?:envoie(?:-moi)?|envoyer|pr[ée]pare|pr[ée]parer)\\s+)?` +
    `(?:(?:[àa]|vers|pour)\\s+)?` +
    `(${EMAIL_RE})(?:\\s+(.+))?$`,
    'i',
  ));
  if (!match) {
    ...
    match = text.match(new RegExp(
      `^${NATURAL_SEND_PREFIX}\\s+` +
      `(?:(?:(?:la|le)\\s+)?fiche(?:\\s+descriptive)?(?:\\s+d[ée]taill[ée]e)?` +
      `(?:\\s+avec\\s+(?:album\\s+de\\s+)?photos?)?|` +
      `(?:(?:tous?|les)\\s+)*(?:docs?|documents?)|` +
      `(?:(?:le|un)\\s+)?dossier)?\\s*` +
      `(?:(?:(?:du|de|pour)(?:\\s+(?:le|la))?|le|la|listing|centris|no|num[ée]ro)\\s+)*` +
  ...
  const centrisMatches = textWithoutEmails(text).match(/\b\d{7,9}\b/g) || [];
...
function parseDirectMatrixBatchRequest(value) {
  const text = cleanMessage(value);
  if (!text || !looksLikeMatrixSendCommand(text)) return null;
  const emailMatches = findEmails(text);
  const centrisMatches = textWithoutEmails(text).match(/\b\d{7,9}\b/g) || [];
  const centrisNums = [...new Set(centrisMatches)];
  if (emailMatches.length !== 1 || centrisNums.length < 2 || centrisNums.length > 10) return null;
  const email = emailMatches[0].replace(/[.,;:!?]+$/, '').toLowerCase();
  const emailIndex = text.toLowerCase().indexOf(email.toLowerCase());
  if (emailIndex < 0 || centrisNums.some(num => text.indexOf(num) > emailIndex)) return null;
  const message = cleanMessage(text.slice(emailIndex + email.length).replace(/^[\s,;:.-]+/, ''));
  return { centrisNums, email, message };
}

function looksLikeMatrixSendWithoutEmail(value) {
  const text = cleanMessage(value);
  if (!new RegExp(`^${NATURAL_SEND_PREFIX}\\b`, 'i').test(text) || new RegExp(EMAIL_RE, 'i').test(text)) return false;
...
function looksLikeMatrixSendCommand(value) {
  ...
  return hasCentrisNumber && (
    new RegExp(`^${NATURAL_SEND_PREFIX}\\b`, 'i').test(text) ||
...
function assertMatrixRequestParserReady() {
  ...
  const requiredCommands = [
    ...
    'Envoie lui 10709767 à shawn@signaturesb.com',
    'Envoie-lui le 10709767 à shawn@signaturesb.com',
    'Tu peux lui envoyer le 10709767 à shawn@signaturesb.com',
    'Fais-lui parvenir le dossier 10709767 à shawn@signaturesb.com',
  ...
  const batch = parseDirectMatrixBatchRequest('Envoie 10709767, 19465925 et 28936167 à shawn@signaturesb.com');
  if (!batch || batch.centrisNums.join(',') !== '10709767,19465925,28936167' ||
      batch.email !== expected.email) {
    throw new Error('MATRIX_BATCH_REQUEST_PARSER_BOOT_SELF_TEST_FAILED');
  }
  return true;
}

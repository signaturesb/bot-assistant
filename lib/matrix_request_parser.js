...
function looksLikeMatrixSendWithoutEmail(value) {
  const text = cleanMessage(value);
  if (!/^(?:envoie(?:-moi)?|envoyer|pr[ée]pare|pr[ée]parer)\b/i.test(text) || new RegExp(EMAIL_RE, 'i').test(text)) return false;
...
function looksLikeMatrixSendCommand(value) {
  ...
  return hasCentrisNumber && (
    /^(?:envoie(?:-moi)?|envoyer|pr[ée]pare|pr[ée]parer)\b/i.test(text) ||

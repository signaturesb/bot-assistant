...
function selectFirstEmailConfirmation({ pending, external, repliedMessageId = 0 } = {}) {
  ...
  const matrixReplyMatches = Boolean(
    external &&
    external.name === 'telecharger_annexes_centris' &&
    replyId > 0 &&
    replyId === Number(external.telegramPreviewMessageId || 0)
  );
  if (pending && external) {
    return matrixReplyMatches
      ? { ok: true, kind: 'external', action: external }
      : { ok: false, reason: 'ambiguous' };
  }
  if (external) {
    // Lorsqu'il n'existe qu'une seule transaction active, le mot exact
    // « envoie » la sélectionne sans obliger Shawn à utiliser Reply dans
    // Telegram. La réponse liée reste nécessaire pour départager deux actions.

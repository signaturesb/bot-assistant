...
function selectFirstEmailConfirmation({ pending, external, repliedMessageId = 0 } = {}) {
  if (!pending && !external) return { ok: false, reason: 'none' };
  const replyId = Number(repliedMessageId || 0);
  const replyMatches = action => Boolean(
    action && replyId > 0 && replyId === Number(action.telegramPreviewMessageId || 0)
  );
  if (pending && external) {
    return replyMatches(external)
      ? { ok: true, kind: 'external', action: external }
      : replyMatches(pending)
        ? { ok: true, kind: 'draft', action: pending }
      : { ok: false, reason: 'ambiguous' };
  }
  if (external) {
    // La nouvelle demande ciblée remplace déjà tout ancien aperçu non envoyé.
    // S'il ne reste qu'une seule transaction, « envoie » confirme exactement
    // celle qui vient d'être affichée, sans imposer une deuxième confirmation.
    return { ok: true, kind: 'external', action: external };
  }
  return { ok: true, kind: 'draft', action: pending };
}

...
â€¢ JAMAIS inventer, corriger ou suggÃ©rer un autre numÃ©ro Centris. RÃ©utiliser uniquement le numÃ©ro exact fourni par Shawn.
...
// rÃ©elle cÃ´tÃ© Matrix avant chaque renouvellement/rÃ©utilisation Playwright.
...
async function centrisGet(path, options = {}, authRetry = 0) {
  ...
  try {
    ...
    if (centrisResponseNeedsLogin(res)) {
      if (authRetry >= 1) throw new Error('Session Centris refusÃ©e aprÃ¨s renouvellement automatique');

...
//   - enregistrerResumeAppel() â€” vocal Telegram rÃ©sumÃ© d'appel
//   - creerDeal() â€” crÃ©ation de deal manuelle ou auto
//
...
function cronTimeout(label, fn, timeoutMs = 120000) {
  return Promise.race([
    ...
    new Promise(res => setTimeout(() => {
      log('WARN', 'CRON', `${label}: TIMEOUT ${timeoutMs/1000}s â€” abandonnÃ©`);
      res();
    }, timeoutMs)),
  ]);
}
...
// l'event loop ou laisser un Ã©tat inconsistant). Combine cronTimeout + catch.
 ...
 *   - shawnConsent: boolean (si true = consent attestÃ© par caller)
  ...
  try {
    ...
    if (r?.ok) {
      ...
      if (html && html.length > 5000) {
        ...
        log('OK', 'TEMPLATE', `Master template chargÃ© ${Math.round(html.length/1024)}KB`);
        return html;
      }
    }
...
â€¢ Pas de rÃ©ponse Ã— 3 â†’ marquer_perdu + ajouter_brevo (nurture)
...
5. âš ï¸ ATTENDRE confirmation de Shawn AVANT d'envoyer pour vrai
   ...
   â†’ Shawn confirme avec: "envoie", "go", "parfait", "ok", "oui", "d'accord", "send"
   â†’ Le systÃ¨me dÃ©tecte ces mots et envoie automatiquement â€” PAS besoin d'appeler un autre outil.
...
ğŸ”¹ traiterNouveauLead(): Gmailâ†’parseâ†’match Dropboxâ†’creerDeal Pipedriveâ†’envoyerDocsAuto
...
ğŸ”¹ envoyerDocsAuto() avec seuils 90/80: â‰¥90 auto, 80-89 attend "envoie", <80 brouillon
...
function getSystemDynamic() {
  ...
  parts.push(
    ...
    `Calculer "demain", "vendredi prochain", "dans 3 jours" Ã€ PARTIR DE ${dateISO}.\n` +
  ...
  // â”â” DÃ‰TECTION AUTO RÃ‰SUMÃ‰ D'APPEL â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
  parts.push(
    `â”â” DÃ‰TECTION RÃ‰SUMÃ‰ D'APPEL (impÃ©ratif â€” vocal Telegram) â”â”\n` +
    `Si le message utilisateur (transcription vocale OU texte) ressemble Ã  un compte-rendu d'appel tÃ©lÃ©phonique avec un client, tu DOIS appeler l'outil enregistrer_resume_appel({transcription: "<texte complet>"}) AUTOMATIQUEMENT, sans demander confirmation.\n\n` +
    `Patterns dÃ©clencheurs:\n` +
    `â€¢ "j'ai parlÃ© avec [Nom]" / "viens de parler Ã  [Nom]"\n` +
    `â€¢ "[Nom] vient d'appeler" / "[Nom] m'a appelÃ©"\n` +
    `â€¢ "rappel de [Nom]" / "discussion avec [Nom]"\n` +
    `â€¢ "[Nom] est intÃ©ressÃ© par X" / "[Nom] veut visiter"\n` +
    `â€¢ "rÃ©sumÃ© d'appel" / "compte-rendu"\n` +
    `â€¢ Tout vocal dÃ©crivant les dÃ©tails d'une conversation client (engagement, budget, prochaine Ã©tape, objections)\n\n` +
    `Passe la transcription COMPLÃˆTE telle quelle dans le param transcription. L'outil:\n` +
    `1. Extrait infos via Haiku (nom, tel, budget, engagement, etc)\n` +
    `2. Cherche client existant Pipedrive (nomâ†’telâ†’Centrisâ†’prÃ©nom)\n` +
    `3. NOUVEAU client â†’ crÃ©e deal + note + activitÃ© (date du jour)\n` +
    `4. CLIENT EXISTANT â†’ ajoute note seulement (rÃ¨gle 1-activitÃ©-par-deal)\n` +
    `5. Pas de nom extrait â†’ renvoie rÃ©sumÃ© sur Telegram pour attribution manuelle\n\n` +
    `NE PAS appeler chercher_prospect ou creer_deal manuellement â€” l'outil gÃ¨re tout.`
  );
...
// â”€â”€â”€ MÃ©moire longue durÃ©e â€” 500 msgs window + Gist backup + Sonnet summary + auto-facts â”€â”€
...
// 3. Gist backup: sauvÃ© toutes les 30s aprÃ¨s modif â†’ survit aux redeploys Render
...
function scheduleHistSave() {
  ...
  // Backup Gist dÃ©bounce 30s (survit redeploys Render)
...
// Gist backup/restore â€” survit aux redeploys Render (disque /data volatil)
...
function extractMemos(text) {
  ...
  if (memos.length) {
    ...
    log('OK', 'MEMO', `${memos.length} fait(s) mÃ©morisÃ©(s) | Gist sync: ${now - lastGistSync < 1000 ? 'immÃ©diat' : 'diffÃ©rÃ©'}`);
  }
...
async function reportBug(titre, description, opts = {}) {
  if (!process.env.GITHUB_TOKEN) {
    log('WARN', 'BUG-TRACKER', `reportBug skipped â€” pas de GITHUB_TOKEN: ${titre}`);
    return null;
  }
...
async function initGistId() {
  if (gistId) { log('OK', 'GIST', `ConfigurÃ©: ${gistId}`); return; }
...
async function creerActivite({ terme, type, sujet, date, heure }) {
  ...
  // ğŸ”„ AUTO-COMPLETE â€” marque les anciennes activitÃ©s open comme done
  // (RÃ¨gle Shawn: 1 active Ã  la fois, ancien complÃ©tÃ© au nouveau suivi)
  const completed = await completerAnciennesActivites(deal.id);
  if (completed > 0) log('OK', 'DEDUP', `${completed} ancienne(s) activitÃ©(s) complÃ©tÃ©e(s) auto sur deal ${deal.id}`);
...
async function creerDeal({ prenom, nom, telephone, email, type, source, centris, note }) {
  ...
  if (!deal?.id) return `âŒ Erreur crÃ©ation deal Pipedrive â€” vÃ©rifie PIPEDRIVE_API_KEY dans Render.`;
...
async function planifierVisite({ prospect, date, adresse }) {
  ...
  // Parser la date â€” utilise ISO si fournie, sinon now+1jour
  let rdvISO = date;
  if (!date.includes('T') && !date.includes('-')) {
    // Date naturelle â€” approximation simple
    rdvISO = new Date(Date.now() + 86400000).toISOString();
  }
  const dateStr = rdvISO.split('T')[0];
  // RÃˆGLE Shawn: pas d'heure par dÃ©faut. Si pas explicite dans rdvISO â†’ null.
  ...
  // ğŸ›¡ï¸ RÃˆGLE 1-activitÃ©-par-deal: complÃ¨te les anciennes AVANT de crÃ©er la visite
  // (planifier une visite = nouvelle Ã©tape du cheminement, l'ancienne devient done auto)
  const completed = await completerAnciennesActivites(deal.id);
  if (completed > 0) log('OK', 'PD', `${completed} ancienne(s) activitÃ©(s) complÃ©tÃ©e(s) sur deal ${deal.id} avant visite`);
  ...
  return `âœ… Visite planifiÃ©e: *${deal.title}*\nğŸ“… ${dateStr}${timeStr ? ' Ã  ' + timeStr : ' (pas d\'heure)'}${adresse ? '\nğŸ“ ' + adresse : ''}\nDeal â†’ Visite prÃ©vue âœ“${completed > 0 ? `\n${completed} ancienne(s) activitÃ©(s) auto-complÃ©tÃ©e(s)` : ''}`;
}
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// AUTO-ENVOI DOCS â€” garantie livraison avec retry + anti-doublon + tracking
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
async function envoyerDocsAuto({ email, nom, centris, dealId, deal, match, _shawnConsent }) {
  // ğŸ”’ KILLSWITCH consent â€” si CONSENT_REQUIRED, refuse tout envoi sauf si
  // l'appelant a explicitement attestÃ© que Shawn a confirmÃ© via Telegram
  // (ex: handler "envoie les docs Ã  X" passe _shawnConsent: false /* legacy consent disabled; one-shot required */).
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      const result = await envoyerDocsProspect(nom || email, email, null, {
        ...
        centrisHint: centris,
        _shawnConsent: false /* legacy consent disabled; one-shot required */, // arrivÃ©s ici = caller a dÃ©jÃ  attestÃ© consent
      });
  ...
  // Alerte Telegram critique ğŸš¨ (P2) + note Pipedrive
  if (dealId) {
    await pdPost('/notes', { deal_id: dealId, content: `âš ï¸ Auto-envoi docs Ã‰CHOUÃ‰ aprÃ¨s 3 tentatives: ${String(lastError).substring(0, 200)}` }).catch(() => null);
  }
  ...
  const alertMsg = [
    ...
    `ğŸ“§ Email: ${email}`,
    `ğŸ¡ Terrain: ${terrain}`,
    `ğŸ” Tentatives: ${maxRetries}/${maxRetries}`,
    ``,
...
<tr><td style="background:${AGENT.couleur};height:4px;font-size:1px;">&nbsp;</td></tr>
  ...
  const logged = await sendEmailLogged({
    ...
    shawnConsent: !!opts._shawnConsent || previewMode, // preview va Ã  shawn@ donc consent implicite
    sendFn: async () => {
      const ctrl = new AbortController();
  ...
  // 9. Note Pipedrive â€” skip en mode preview (c'est juste un preview, pas une vraie livraison)
  ...
  const noteContent = `Documents envoyÃ©s Ã  ${realToEmail} (${new Date().toLocaleString('fr-CA', { timeZone: 'America/Toronto' })}):\n${ok.map(d => `â€¢ ${d.name}`).join('\n')}${convResult?.imagesMerged > 0 ? `\n(${convResult.imagesMerged} photos combinÃ©es en 1 PDF)` : ''}${convertedSkipped?.length > 0 ? `\nFichiers non convertibles skipped: ${convertedSkipped.map(s=>s.name).join(', ')}` : ''}`;
  // IDEMPOTENCY: vÃ©rifier si une note "Documents envoyÃ©s Ã  <email>" existe
  // dÃ©jÃ  dans les 24h pour ce deal â€” Ã©vite 3 notes identiques si retry.
  let skipNote = false;
  if (deal.id) {
    try {
      ...
      const dupFound = items.some(it => {
        ...
        return ts > dayAgo && c.includes(`Documents envoyÃ©s Ã  ${realToEmail}`);
      });
      if (dupFound) { skipNote = true; log('INFO', 'PIPEDRIVE', `Note idempotent: existe dÃ©jÃ  <24h pour ${realToEmail} deal #${deal.id}`); }
  ...
  const noteLabel = skipNote
    ? 'ğŸ“ Note Pipedrive skip (existe dÃ©jÃ  <24h)'
    : (noteRes?.data?.id ? 'ğŸ“ Note Pipedrive ajoutÃ©e' : 'âš ï¸ Note Pipedrive non crÃ©Ã©e');
  log('OK', 'DOCS', `[STEP 9/9] âœ… DONE (${Date.now()-_t0}ms total) â€” ${ok.length} doc(s) envoyÃ©s Ã  ${realToEmail} | note=${skipNote?'skip-dup':(noteRes?.data?.id?'created':'fail')}`);
...
async function centrisLogin() {
  ...
  // PRIORITÃ‰ 1 â€” Nouveau flow OAuth Auth0 avec MFA SMS (Centris a migrÃ© vers Ã§a)
  // L'ancien flow form-based Ã  /fr/connexion ne marche plus pour le portail Matrix.
  // centrisOAuthLoginWithMFA set centrisSession directement Ã  l'intÃ©rieur si OK.
  try {
    const oauth = await centrisOAuthLoginWithMFA({ mfaTimeoutMs: 120000 });
    if (oauth?.ok && centrisSession?.cookies) {
      log('OK', 'CENTRIS', `OAuth Auth0 + MFA SMS âœ“ (agent: ${user}, ${oauth.cookieCount} cookies)`);
      return true;
    }
    // DÃ©gradation propre: si MFA_REQUIRED â†’ log clair, retourne false (pas crash)
    // L'appelant (CUA driver, telechargerFiche etc) sait qu'il doit fallback ailleurs.
    if (oauth?.mfaRequired) {
      log('WARN', 'CENTRIS', `MFA_REQUIRED â€” login en mode dÃ©gradÃ©. Solutions: (1) set CENTRIS_TOTP_SECRET env var, (2) dÃ©marre sms-bridge LaunchAgent Mac, (3) lance LaunchAgent centris-auto-login pour push cookies via Playwright`);
      return false;
    }
    log('WARN', 'CENTRIS', `OAuth flow Ã©chouÃ©: ${oauth?.error || 'unknown'} â€” fallback form-based`);
  } catch (e) {
    log('WARN', 'CENTRIS', `OAuth flow exception: ${e.message?.substring(0, 100)} â€” fallback form-based`);
  }

  // FALLBACK â€” Ancien flow form-based (peut encore marcher pour certains comptes)
  try {
    ...
    // DÃ©tecter le succÃ¨s (redirect 302, cookie auth, ou header Location)
    ...
    if (isOk) {
      ...
      log('OK', 'CENTRIS', `ConnectÃ© âœ“ (code agent: ${user})`);
      return true;
    }

    log('WARN', 'CENTRIS', `Login: HTTP ${loginRes.status} â€” location: ${location.substring(0,80)}`);
    return false;
  } catch (e) {
...
<p style="margin:16px 0;color:#cccccc;font-size:14px;line-height:1.6;">N'hÃ©sitez pas si vous avez des questions â€” je suis disponible au <strong style="color:${AGENT.couleur};">${AGENT.telephone}</strong>.</p>`;
  ...
  const sent = await sendEmailLogged({
    ...
    shawnConsent: false /* legacy consent disabled; one-shot required */, // consent attestÃ© par la commande explicite
    sendFn: () => fetch('https://gmail.googleapis.com/gmail/v1/users/me/messages/send', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ raw }),
...
const TOOLS = [
  ...
  { name: 'creer_activite',         description: 'CrÃ©er une activitÃ©/tÃ¢che/rappel pour un deal. Types: appel, email, rÃ©union, tÃ¢che, visite. UTILISE LA DATE COURANTE DU SYSTEM PROMPT (jamais deviner l\'annÃ©e). RÃˆGLES ABSOLUES: (1) ne JAMAIS passer "heure" sauf si Shawn demande explicitement une heure. (2) ZÃ‰RO sujet gÃ©nÃ©rique type "Suivi contact/prospect", "Appeler contact/prospect", "Rappeler le prospect" â€” le bot refuse. Toujours donner un sujet spÃ©cifique au client/dossier (ex: "Appel Marie - terrain Rawdon"). (3) Pas de tel ni email connu = NE PAS crÃ©er d\'activitÃ© de suivi, juste une note.', input_schema: { type: 'object', properties: { terme: { type: 'string', description: 'Nom du prospect' }, type: { type: 'string', description: 'appel, email, rÃ©union, tÃ¢che, visite' }, sujet: { type: 'string', description: 'Sujet SPÃ‰CIFIQUE â€” JAMAIS gÃ©nÃ©rique. Doit nommer le client + l\'action concrÃ¨te.' }, date: { type: 'string', description: 'Format STRICT YYYY-MM-DD (ex: 2026-04-26). Calculer Ã  partir de la date courante du system prompt.' }, heure: { type: 'string', description: 'OPTIONNEL â€” Format HH:MM (ex: 14:00). NE PAS PASSER sauf si Shawn demande explicitement une heure.' } }, required: ['terme', 'type'] } },
  ...
  { name: 'envoyer_email',       description: 'PrÃ©parer un brouillon email pour approbation de Shawn. Affiche le brouillon complet â€” il N\'EST PAS envoyÃ© tant que Shawn ne confirme pas avec "envoie", "go", "ok", "parfait", "d\'accord", etc.', input_schema: { type: 'object', properties: { to: { type: 'string', description: 'Adresse email du destinataire' }, toName: { type: 'string', description: 'Nom du destinataire' }, sujet: { type: 'string', description: 'Objet de l\'email' }, texte: { type: 'string', description: 'Corps de l\'email â€” texte brut, style Shawn, vouvoiement, max 3 paragraphes courts.' } }, required: ['to', 'sujet', 'texte'] } },
  ...
  // â”€â”€ Brevo â”€â”€
  { name: 'ajouter_brevo',  description: 'Ajouter/mettre Ã  jour un contact dans Brevo. Utiliser quand deal perdu â†’ nurture mensuel, ou nouveau contact Ã  ajouter.', input_schema: { type: 'object', properties: { email: { type: 'string' }, prenom: { type: 'string' }, nom: { type: 'string' }, telephone: { type: 'string' }, liste: { type: 'string', description: 'prospects, acheteurs, vendeurs (dÃ©faut: prospects)' } }, required: ['email'] } },
  ...
  // â”€â”€ Diagnostics â”€â”€
  { name: 'tester_dropbox',  description: 'Tester la connexion Dropbox et diagnostiquer les problÃ¨mes de tokens. Utiliser quand Dropbox semble brisÃ©.', input_schema: { type: 'object', properties: {} } },
  { name: 'voir_template_dropbox', description: 'Lire les informations du master template email depuis Dropbox. Pour vÃ©rifier les placeholders disponibles.', input_schema: { type: 'object', properties: {} } },

  // â”€â”€ Firecrawl (scraping municipal) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  { name: 'scraper_site_municipal', description: 'Scraper le site d\'une municipalitÃ© quÃ©bÃ©coise pour obtenir rÃ¨glements de zonage, marges latÃ©rales, permis, taxes. Cache 30j. Fallback tÃ©lÃ©phone auto si scrape Ã©choue. Villes: sainte-julienne, rawdon, chertsey, saint-calixte, saint-jean-de-matha, saint-didace, matawinie, d-autray.', input_schema: { type: 'object', properties: { ville: { type: 'string', description: 'Nom ville slug (sainte-julienne, rawdon, chertsey, saint-calixte, saint-jean-de-matha, saint-didace, matawinie, d-autray)' }, sujet: { type: 'string', enum: ['zonage', 'urbanisme', 'permis', 'taxes', 'riveraine'], description: 'Type info (dÃ©faut zonage)' } }, required: ['ville'] } },
  { name: 'scraper_url', description: 'Scraper n\'importe quelle URL et extraire markdown (rÃ¨glements, PDFs convertis, pages gouv). Utiliser mots_cles pour filtrer la section pertinente.', input_schema: { type: 'object', properties: { url: { type: 'string', description: 'URL complÃ¨te https://...' }, mots_cles: { type: 'array', items: { type: 'string' }, description: 'Mots-clÃ©s pour filtrer la section (ex: ["marge","latÃ©rale","recul"])' } }, required: ['url'] } },
  // â”€â”€ Recherche web temps rÃ©el (Perplexity Sonar) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  { name: 'recherche_web', description: 'Recherche web temps rÃ©el avec sources citÃ©es. Pour stats marchÃ© immobilier QC, taux hypothÃ©caires actuels, nouvelles rÃ¨gles OACIQ/AMF, comparables rÃ©cents. NÃ©cessite PERPLEXITY_API_KEY env var.', input_schema: { type: 'object', properties: { question: { type: 'string', description: 'Question naturelle (ex: "tendance prix terrains LanaudiÃ¨re 2026", "taux hypothÃ©caire Desjardins aujourd\'hui")' } }, required: ['question'] } },
  // â”€â”€ TÃ©lÃ©chargement PDF + scraping avancÃ© â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  { name: 'telecharger_pdf', description: 'TÃ©lÃ©charge un PDF depuis n\'importe quelle URL et l\'envoie direct sur Telegram Ã  Shawn. Utile pour rÃ©cupÃ©rer rapports municipaux, rÃ¨glements, fiches MLS, certificats de localisation, plans cadastraux. Max 25MB. Retourne URL + taille + envoi confirmÃ©.', input_schema: { type: 'object', properties: { url: { type: 'string', description: 'URL complÃ¨te vers PDF (ex: https://ville.qc.ca/.../zonage.pdf)' }, titre: { type: 'string', description: 'OPTIONNEL â€” titre/lÃ©gende pour le PDF dans Telegram' } }, required: ['url'] } },
  { name: 'scraper_avance', description: 'Scrape une URL + extrait automatiquement TOUS les liens PDF trouvÃ©s. Utile pour explorer un site municipal/gouvernemental oÃ¹ les docs sont en PDF (ex: page urbanisme avec liens vers rÃ¨glements, plans, formulaires). Retourne contenu + liste PDFs avec option de les tÃ©lÃ©charger.', input_schema: { type: 'object', properties: { url: { type: 'string', description: 'URL Ã  scraper' }, mots_cles: { type: 'array', items: { type: 'string' }, description: 'OPTIONNEL â€” filtrer le contenu par mots-clÃ©s' }, telecharger_pdfs: { type: 'boolean', description: 'OPTIONNEL â€” si true, download auto les PDFs trouvÃ©s (max 5)' } }, required: ['url'] } },
  { name: 'scraper_pdf_universel', description: 'SCRAPER ULTIME PDF â€” cascade 3 niveaux (HTTP direct â†’ Firecrawl â†’ Browserless stealth) avec bypass consent walls, anti-bot, JS-rendered. UTILISER quand scraper_avance Ã©choue ou pour sites qui bloquent (cookies wall, Cloudflare, JS SPAs). Auto-click "Accepter cookies"+"J\'accepte". TÃ©lÃ©charge auto les PDFs trouvÃ©s.', input_schema: { type: 'object', properties: { url: { type: 'string', description: 'URL Ã  scraper' }, mot_cle_filtre: { type: 'string', description: 'OPTIONNEL â€” filtre PDFs par mot-clÃ© (ex: "zonage", "rÃ¨glement", "plan")' }, max_pdfs: { type: 'number', description: 'OPTIONNEL â€” max PDFs Ã  tÃ©lÃ©charger (dÃ©faut 5)' } }, required: ['url'] } },
  { name: 'recherche_documents', description: 'COMBINAISON puissante: cherche sur le web (Perplexity) + scrape les sources trouvÃ©es (Firecrawl) + extrait/tÃ©lÃ©charge les PDFs pertinents. Pour "trouve-moi le rÃ¨glement de zonage X en PDF", "documents officiels MRC LanaudiÃ¨re sur Y", "fiche technique propriÃ©tÃ© Z". NÃ©cessite PERPLEXITY_API_KEY + FIRECRAWL_API_KEY.', input_schema: { type: 'object', properties: { question: { type: 'string', description: 'Ce que tu cherches (ex: "rÃ¨glement bande riveraine Saint-Calixte PDF")' }, max_resultats: { type: 'number', description: 'OPTIONNEL â€” combien de sources scraper (dÃ©faut 3, max 5)' } }, required: ['question'] } },
  // â”€â”€ RÃ©sumÃ© d'appel tÃ©lÃ©phonique (vocal Telegram â†’ Pipedrive auto) â”€â”€â”€â”€â”€â”€â”€â”€
  { name: 'enregistrer_resume_appel', description: 'Analyse une transcription d\'appel tÃ©lÃ©phonique (vocal Telegram), extrait via Haiku les infos clÃ©s (nom client, budget, engagement chaud/tiÃ¨de/froid, objections, prochaine Ã©tape) et crÃ©e/enrichit le deal Pipedrive: NOUVEAU client â†’ crÃ©e person + deal + note rÃ©sumÃ© + activitÃ© de suivi (date du jour). CLIENT EXISTANT â†’ ajoute juste la note rÃ©sumÃ©. Ã€ UTILISER AUTOMATIQUEMENT quand Shawn envoie un vocal qui dÃ©crit un appel (patterns: "j\'ai parlÃ© avec X", "vient d\'appeler", "rappel de X", "discussion avec X", "X m\'a appelÃ©", "rÃ©sumÃ© d\'appel", "X est intÃ©ressÃ© par"). NE PAS demander confirmation â€” exÃ©cuter directement.', input_schema: { type: 'object', properties: { transcription: { type: 'string', description: 'Texte transcrit du vocal â€” passer la transcription Whisper complÃ¨te, telle quelle' } }, required: ['transcription'] } },
  // â”€â”€ Centris fiche download â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  { name: 'telecharger_fiche_centris', description: 'TÃ©lÃ©charge la fiche dÃ©taillÃ©e PDF d\'un listing Centris (peu importe quel courtier l\'a inscrit) via portail courtier authentifiÃ© de Shawn, et envoie par courriel au destinataire. Cas d\'usage: "envoie la fiche du #12345678 Ã  client@email.com". Toi en Cc auto. NÃ©cessite CENTRIS_USER+CENTRIS_PASS.', input_schema: { type: 'object', properties: { centris_num: { type: 'string', description: 'NumÃ©ro Centris/MLS du listing (7-9 chiffres)' }, email_destination: { type: 'string', description: 'Email oÃ¹ envoyer la fiche' }, cc: { type: 'string', description: 'OPTIONNEL â€” CCs additionnels (sÃ©parÃ©s par virgules)' }, message_perso: { type: 'string', description: 'OPTIONNEL â€” message personnalisÃ© dans le courriel (sinon template Shawn standard)' } }, required: ['centris_num', 'email_destination'] } },
  { name: 'envoyer_fiche_centris_native', description: 'MEILLEUR pour envoyer fiche Centris au client: utilise l\'UI Matrix native (Imprimer â†’ DÃ©taillÃ© client avec album de photos â†’ Envoyer par courriel). Le PDF est OFFICIEL Centris (photos haute rÃ©solution + signature Shawn intÃ©grÃ©e), expÃ©diÃ© par l\'infra Centris. PRIVILÃ‰GIER pour tout envoi Ã  un client. Toi en Cc auto. Cas d\'usage: "envoie la fiche du #12345678 Ã  client@email.com".', input_schema: { type: 'object', properties: { centris_num: { type: 'string', description: 'NumÃ©ro Centris/MLS (7-9 chiffres)' }, email: { type: 'string', description: 'Email destinataire' }, cc: { type: 'string', description: 'OPTIONNEL â€” dÃ©faut shawn@signaturesb.com' }, sujet: { type: 'string', description: 'OPTIONNEL â€” sujet email (dÃ©faut: "PropriÃ©tÃ© Centris #N")' }, message: { type: 'string', description: 'OPTIONNEL â€” corps du courriel (dÃ©faut: template standard)' }, format: { type: 'string', enum: ['detaille_client_album_imperial', 'detaille_client_imperial', 'detaille_courtier_album_imperial', 'sommaire_imperial'], description: 'OPTIONNEL â€” format rapport (dÃ©faut album photos)' } }, required: ['centris_num', 'email'] } },
  { name: 'envoyer_tous_documents_zone', description: 'Envoie TOUS les documents d\'un listing Centris (Fiche dÃ©taillÃ©e + DV + Taxes municipales + Taxes scolaires + Cert localisation + Plans) au destinataire via Zone Centris (zone.centris.ca). Identifie automatiquement le courtier inscripteur. Cas d\'usage: "envoie TOUS les docs du #12345678 Ã  client@email.com". UNE seule action partage tous les documents uploadÃ©s par le courtier inscripteur. PLUS COMPLET que envoyer_fiche_centris_native (qui envoie juste la fiche). RECOMMANDÃ‰: utiliser verifier_listing_centris d\'abord pour vÃ©rifier le courtier + liste des docs avant envoi (mode preview sans risque).', input_schema: { type: 'object', properties: { centris_num: { type: 'string', description: 'NumÃ©ro Centris (7-9 chiffres)' }, email: { type: 'string', description: 'Email destinataire' }, m_envoyer_copie: { type: 'boolean', description: 'OPTIONNEL â€” me mettre en copie (dÃ©faut false)' }, langue: { type: 'string', enum: ['fr', 'en'], description: 'OPTIONNEL â€” langue email (dÃ©faut fr)' }, message: { type: 'string', description: 'OPTIONNEL â€” message custom (dÃ©faut: message Centris standard)' } }, required: ['centris_num', 'email'] } },
  { name: 'verifier_listing_centris', description: 'PREVIEW SANS ENVOI â€” vÃ©rifie qui est le courtier inscripteur d\'un listing Centris ET liste tous les documents disponibles. ZÃ‰RO email envoyÃ©, juste reconnaissance. Cas d\'usage: "qui vend le #12345678" ou "quels docs sont dispos sur le #12345678" ou "check #12345678 avant d\'envoyer". Ã€ utiliser AVANT envoyer_tous_documents_zone pour valider sans risque. Retourne: nom courtier + agence + tÃ©lÃ©phone + liste des N documents avec tailles.', input_schema: { type: 'object', properties: { centris_num: { type: 'string', description: 'NumÃ©ro Centris (7-9 chiffres)' } }, required: ['centris_num'] } },
  { name: 'telecharger_docs_centris_complet', description: 'TOUT-EN-UN: envoie au client la fiche Centris officielle (PDF portail courtier) + TOUS les docs Dropbox matchant (match auto par Centris#). Cas d\'usage: "Envoie tout ce qui est dispo sur #12345678 Ã  client@email.com". Toi en Cc auto sur les 2 envois. Le client reÃ§oit 2 emails (1 avec fiche, 1 avec docs Dropbox).', input_schema: { type: 'object', properties: { centris_num: { type: 'string', description: 'NumÃ©ro Centris (7-9 chiffres)' }, email_destination: { type: 'string', description: 'Email du client' }, cc: { type: 'string', description: 'OPTIONNEL â€” CCs additionnels' }, message_perso: { type: 'string', description: 'OPTIONNEL â€” message dans email fiche' } }, required: ['centris_num', 'email_destination'] } },
  { name: 'analyser_zonage_adresse', description: 'Trouve et envoie la grille de zonage PDF officielle pour une adresse LanaudiÃ¨re. Scrape page urbanisme municipal â†’ trouve liens PDF zonage â†’ tÃ©lÃ©charge â†’ envoie dans Telegram comme document. Optionnellement forward au client par email avec Cc Shawn. Cas d\'usage: "Marges de construction au 123 Ch. Lac Gratten Rawdon" ou "Grille zonage 456 Rue Sarine Sainte-Julienne, envoie Ã  client@email.com".', input_schema: { type: 'object', properties: { adresse: { type: 'string', description: 'Adresse complÃ¨te avec ville (ex: "123 Chemin Lac Gratten, Rawdon")' }, forward_email: { type: 'string', description: 'OPTIONNEL â€” email client si demande explicite forward (Shawn dit "envoie Ã  X")' } }, required: ['adresse'] } },
  { name: 'telecharger_annexes_centris', description: 'RÃ©cupÃ¨re TOUTES les annexes Centris d\'un listing via portail courtier authentifiÃ©: DÃ©claration Vendeur (DV), certificat de localisation, plans cadastraux, rapport inspection, etc. Tout ce qui est dans la section "Annexes" du listing Matrix. Cas d\'usage: "Donne-moi la DV du #12345678" ou "Toutes les annexes Centris pour #12345678 Ã  client@email.com".', input_schema: { type: 'object', properties: { centris_num: { type: 'string', description: 'NumÃ©ro Centris/MLS (7-9 chiffres)' }, email_destination: { type: 'string', description: 'OPTIONNEL â€” email client pour forward avec Cc Shawn. Si vide: envoi dans Telegram seulement.' }, filtre: { type: 'string', description: 'OPTIONNEL â€” filtrer par mot-clÃ© dans nom annexe (ex: "DV", "dÃ©claration", "localisation", "plan"). Si vide: toutes les annexes.' } }, required: ['centris_num'] } },
];

// Cache les tools (statiques) â€” Anthropic prompt caching sur le dernier tool
// = cache la totalitÃ© de la liste TOOLS (envoyÃ©e Ã  chaque call). Ã‰conomise ~90%
// du coÃ»t input_tokens des tools. Cache TTL: 5 min (renouvelÃ© Ã  chaque appel).
...
async function executeTool(name, input, chatId, userMessage = '') {
  try {
    ...
    switch (name) {
      ...
      case 'voir_visites': {
        const visites = loadJSON(VISITES_FILE, []);
        if (!visites.length) return 'ğŸ“… Aucune visite planifiÃ©e.';
        const now = Date.now();
        const futures = visites.filter(v => new Date(v.date).getTime() > now - 3600000); // +1h passÃ©e
        if (!futures.length) return 'ğŸ“… Aucune visite Ã  venir (toutes passÃ©es).';
        const today = new Date().toDateString();
        let txt = `ğŸ“… *Visites planifiÃ©es â€” ${futures.length} total*\n\n`;
        for (const v of futures.sort((a, b) => new Date(a.date) - new Date(b.date))) {
          ...
          txt += `${isToday ? 'ğŸ”´ AUJOURD\'HUI' : 'ğŸ“†'} *${v.nom}*\n${dateStr} Ã  ${timeStr}${v.adresse ? '\nğŸ“ ' + v.adresse : ''}\n\n`;
        }
        return txt.trim();
      }
      ...
      // ğŸ›‘ SHAWN 2026-06-09 RÃˆGLE ABSOLUE: bot a SEULEMENT droit CREATE deal + CREATE activitÃ©.
      // JAMAIS DELETE Pipedrive. Si vraiment nÃ©cessaire â†’ Shawn fait dans Pipedrive UI directement.
      case 'supprimer_activite':      return `ğŸ›‘ BLOQUÃ‰ â€” Shawn 2026-06-09: le bot n'a plus le droit de supprimer dans Pipedrive. Fais-le dans Pipedrive UI.`;
      case 'deplacer_activite':       return await deplacerActivite(input);
      case 'fusionner_deals':         return `ğŸ›‘ BLOQUÃ‰ â€” fusion = suppression d'un deal. Fais dans Pipedrive UI.`;
      case 'fusionner_personnes':     return `ğŸ›‘ BLOQUÃ‰ â€” fusion = suppression. Fais dans Pipedrive UI.`;
      case 'supprimer_deal':          return `ğŸ›‘ BLOQUÃ‰ â€” Shawn 2026-06-09: bot ne supprime pas dans Pipedrive. Fais dans Pipedrive UI.`;
      case 'supprimer_personne':      return `ğŸ›‘ BLOQUÃ‰ â€” bot ne supprime pas dans Pipedrive. Fais dans Pipedrive UI.`;
      case 'supprimer_note':          return `ğŸ›‘ BLOQUÃ‰ â€” bot ne supprime pas dans Pipedrive. Fais dans Pipedrive UI.`;
      ...
      case 'chercher_comparables': {
        ...
        const listings = res;
        const fmt = n => n ? `${Number(n).toLocaleString('fr-CA')} $` : 'â€”';
        const fmtS = n => n ? `${Number(n).toLocaleString('fr-CA')} piÂ²` : 'â€”';
        const fmtPp = (p,s) => (p&&s&&s>100) ? `${(p/s).toFixed(2)} $/piÂ²` : 'â€”';
        const avecPrix = listings.filter(l=>l.prix>1000);
        const prixMoy = avecPrix.length ? Math.round(avecPrix.reduce((s,l)=>s+l.prix,0)/avecPrix.length) : 0;
        let txt = `ğŸ“Š *${listings.length} ${input.type||'terrain'}(s) vendus â€” ${input.ville} â€” ${input.jours||14}j*\n`;
        if (prixMoy) txt += `Prix moyen: *${fmt(prixMoy)}*\n`;
        txt += '\n';
        listings.slice(0,12).forEach((l,i) => {
          txt += `${i+1}. ${l.adresse||'Adresse N/D'}${l.mls?' (#'+l.mls+')':''}\n`;
          txt += `   ${fmt(l.prix)} Â· ${fmtS(l.superficie)} Â· ${fmtPp(l.prix,l.superficie)}${l.dateVente?' Â· '+l.dateVente:''}\n`;
        });
        txt += `\n_Source: Pipedrive (deals gagnÃ©s)_`;
        if (listings.length > 12) txt += ` Â· _+ ${listings.length-12} autres â€” dis "envoie rapport" pour tout par email._`;
        else txt += ` Â· _Dis "envoie rapport" pour recevoir par email avec template Signature SB._`;
        return txt;
      }
      case 'proprietes_en_vigueur': {
        ...
        const fmt = n => n ? `${Number(n).toLocaleString('fr-CA')} $` : 'â€”';
        const fmtS = n => n ? `${Number(n).toLocaleString('fr-CA')} piÂ²` : 'â€”';
        let txt = `ğŸ¡ *${res.length} ${input.type||'terrain'}(s) en vigueur â€” ${input.ville}*\nSource: Centris.ca (agent ${process.env.CENTRIS_USER||''})\n\n`;
        res.slice(0,15).forEach((l,i) => {
          txt += `${i+1}. ${l.adresse||'N/D'}${l.mls?' (#'+l.mls+')':''}\n   ${fmt(l.prix)} Â· ${fmtS(l.superficie)}\n`;
        });
        if (res.length > 15) txt += `\n_+ ${res.length-15} autres â€” dis "envoie rapport actifs ${input.ville}" pour tout par email._`;
        return txt;
      }
      ...
      case 'envoyer_email': {
        // Stocker le brouillon â€” ne PAS envoyer encore
        pendingEmails.set(chatId, { to: input.to, toName: input.toName, sujet: input.sujet, texte: input.texte });
        return `ğŸ“§ *BROUILLON EMAIL â€” EN ATTENTE D'APPROBATION*\n\n*Ã€:* ${input.toName ? input.toName + ' <' + input.to + '>' : input.to}\n*Objet:* ${input.sujet}\n\n---\n${input.texte}\n---\n\nğŸ’¬ Dis *"envoie"* pour confirmer, ou modifie ce que tu veux.`;
      }
      ...
      case 'send_dropbox_file': {
        ...
        if (!file) return `Erreur: impossible de tÃ©lÃ©charger ${input.path}`;
        await bot.sendDocument(chatId, file.buffer, { caption: input.caption || '' }, { filename: file.filename });
        return `âœ… Fichier "${file.filename}" envoyÃ©.`;
      }
      case 'read_bot_file': {
      ...
      case 'write_bot_file': {
        ...
        return `âœ… "${input.filename}" sauvegardÃ©.`;
      }
      case 'refresh_contexte_session': {
        await loadSessionLiveContext();
        return sessionLiveContext
          ? `âœ… *Session rechargÃ©e* â€” ${Math.round(sessionLiveContext.length/1024)}KB\n\n*Contexte actuel:*\n${sessionLiveContext.substring(0, 400)}...`
          : 'âš ï¸ SESSION_LIVE.md vide ou inaccessible.';
      }
ÛM¼Ó†òµë(š+myÓ¢c¢°¢v—B&÷Bç6VæDÖW76vR†×6ræ6†Bæ–BÀ¢	ùI¤Æöv–â6VçG&—2ôWF‚²Ôd¥Ææ°¢'&–FvRÖ24Õ3¢G¶'&–FvTÆ—fRò	ùú"7F–br¢~)ªûˆò2FR†V'F&VBÃÖ–âwÕÆæ°¢ôÆR&÷Bf&V6Wfö—"Vâ4Õ26öFR(i"'&–FvRf÷'v&B(i"–æ¦V7F–öâWFòåõÆæ°¢õF–Væ6Rã3Óc2Â7W'F÷WB÷W"ÆR4Õ5öÀ¢²'6UöÖöFS¢tÖ&¶F÷vârĞ¢“°¢&÷Bç6VæD6†D7F–öâ†×6ræ6†Bæ–BÂwG—–ærr’æ6F6‚‚‚’Óâ·Ò“°¢6öç7BG—–ærÒ6WD–çFW'fÂ‚‚’Óâ&÷Bç6VæD6†D7F–öâ†×6ræ6†Bæ–BÂwG—–ærr’æ6F6‚‚‚’Óâ·Ò’ÂCS“°¢G'’°¢6öç7B&W7VÇBÒv—B6VçG&—4ôWF„Æöv–åv—F„Ôd‡²ÖfF–ÖV÷WD×3¢#Ò“°¢6ÆV$–çFW'fÂ‡G—–ær“°¢–b‡&W7VÇBæö²’°¢v—B&÷Bç6VæDÖW76vR†×6ræ6†Bæ–BÀ¢)ÈR¤Æöv–â6VçG&—2ô²¥Ææ°¢6öö¶–W3¢G·&W7VÇBæ6öö¶–T6÷VçGÒ+r6W76–öâfÆ–FR#F…Ææ°¢GRWW‚Ö–çFVæçBWF–Æ—6W"Æöf–6†RÂ3âÆVÖ–ÃåÆÀ¢²'6UöÖöFS¢tÖ&¶F÷vârĞ¢“°¢VF—DÆötWfVçB‚v6VçG&—2rÂvöWF‚ÖÆöv–â×7V66W72rÂ²6öö¶–W3¢&W7VÇBæ6öö¶–T6÷VçBÒ“°¢ÒVÇ6R°¢v—B&÷Bç6VæDÖW76vR†×6ræ6†Bæ–BÂ)ØÂ¤Æöv–â:–6†÷\:“¢¢G·&W7VÇBæW'&÷'ÖÂ²'6UöÖöFS¢tÖ&¶F÷vârÒ“°¢VF—DÆötWfVçB‚v6VçG&—2rÂvöWF‚ÖÆöv–âÖf–ÆVBrÂ²W'&÷#¢&W7VÇBæW'&÷"Ò“°¢Ğ¢Ò6F6‚†R’°¢6ÆV$–çFW'fÂ‡G—–ær“°¢v—B&÷Bç6VæDÖW76vR†×6ræ6†Bæ–BÂ)ØÂW†6WF–öã¢G¶RæÖW76vSòç7V'7G&–ærƒÂ#—Ö“°¢Ğ¢Ò“° ¢òòöf–6†RÂ66VçG&—3âÆVÖ–Ãâ¶ÖW76vU÷W'6õÒ(	BL:–Ì:–6†&vRf–6†R6VçG&—2²Vçfö–P¢òò62W6vS¢GRW27W"ÆRFW'&–âÂ6Æ–VçBFVÖæFR–æfò7W"VâWG&RÆ—7F–ær0¢òò:Fö’(i"öf–6†R#3CScs‚6Æ–VçDvÖ–Âæ6öÒ(i"&÷BfWF6‚²Vçfö–RVâ2à¢&÷BæöåFW‡B‚õåÂöf–6†UÇ2²…ÆG³rÃ—Ò•Ç2²…Å2´Å2²’ƒó¥Ç2²‚â²’“òö’Â7–æ2†×6rÂÖF6‚’Óâ°¢–b‚—4ÆÆ÷vVB†×6r’’&WGW&ã°¢6öç7BçVÒÒÖF6…³Ó°¢6öç7BVÖ–ÂÒÖF6…³%Ó°¢6öç7BÖW76vU÷W'6òÒÖF6…³5ÓòçG&–Ò‚’ÇÂçVÆÃ°¢v—B&÷Bç6VæDÖW76vR†×6ræ6†Bæ–BÂ	ù:R¤f–6†R6VçG&—22G¶çV×Ò¢(i"G¶VÖ–ÇÕÆåôÆöv–â6VçG&—2²F÷væÆöB²Vçfö’ƒÓ32•öÂ²'6UöÖöFS¢tÖ&¶F÷vârÒ“°¢&÷Bç6VæD6†D7F–öâ†×6ræ6†Bæ–BÂwG—–ærr’æ6F6‚‚‚’Óâ·Ò“°¢6öç7BG—–ærÒ6WD–çFW'fÂ‚‚’Óâ&÷Bç6VæD6†D7F–öâ†×6ræ6†Bæ–BÂwG—–ærr’æ6F6‚‚‚’Óâ·Ò’ÂCS“°¢G'’°¢6öç7B&W7VÇBÒv—BFVÆV6†&vW$f–6†T6VçG&—2‡²6VçG&—5öçVÓ¢çVÒÂVÖ–ÅöFW7F–æF–öã¢VÖ–ÂÂÖW76vU÷W'6òÒ“°¢6ÆV$–çFW'fÂ‡G—–ær“°¢v—B&÷Bç6VæDÖW76vR†×6ræ6†Bæ–BÂ7G&–ær‡&W7VÇB’ç7V'7G&–ærƒÂC’Â²'6UöÖöFS¢tÖ&¶F÷vârÒ’æ6F6‚‚‚’Óà¢&÷Bç6VæDÖW76vR†×6ræ6†Bæ–BÂ7G&–ær‡&W7VÇB’ç7V'7G&–ærƒÂC’ç&WÆ6R‚õ²¥öÒörÂrr’’æ6F6‚‚‚’Óâ·Ò¢“°¢Ò6F6‚†R’°¢6ÆV$–çFW'fÂ‡G—–ær“°¢v—B&÷Bç6VæDÖW76vR†×6ræ6†Bæ–BÂ)ØÂW'&WW#¢G¶RæÖW76vSòç7V'7G&–ærƒÂ3—Ö“°¢Ğ¢Ò“° ¢òòö–æfòÂ46VçG&—2÷RG&W76Sâ(	BD4„$ô$B6ö×ÆWBBwVæR&÷&œ:—L:’‡FW'&–âÖöFR¢òò÷W"6†vâ7W"ÆRFW'&–âfV2Vâ6Æ–VçC¢F÷WBVâ6öÖÖæFRÂ&ÆÆVÂ6ÆÇ2à¢òò&WF÷W&æRFVÂ—VG&—fR²F÷76–W"G&÷&÷‚²†÷F÷2²–æfò¦öævR²6ö×&&ÆW2à¢&÷BæöåFW‡B‚õåÂö–æfõÇ2²‚â²’ö’Â7–æ2†×6rÂÖF6‚’Óâ°¢–b‚—4ÆÆ÷vVB†×6r’’&WGW&ã°¢6öç7BVW'’ÒÖF6…³ÒçG&–Ò‚“°¢6öç7B—46VçG&—2ÒõåÆG³rÃ—ÒBòçFW7B‡VW'’“°¢v—B&÷Bç6VæDÖW76vR†×6ræ6†Bæ–BÂ	ùHâ¥&V6†W&6†R6ö×Ì:‡FS¢¢ÆG·VW'—ÕÆÆåòG¶—46VçG&—2òt6VçG&—22L:—FV7L:’r¢w&V6†W&6†R"G&W76RöæöÒwÕöÂ²'6UöÖöFS¢tÖ&¶F÷vârÒ“° ¢òò$ÄÌ8”Ä•4D”ôâ(	BF÷WFW2ÆW2Æöö·W2Vâ&ÆÌ:†ÆRƒ2ÓW2F÷FÂRÆ–WRFRW2¢6öç7BCÒFFRææ÷r‚“°¢6öç7BF6·2Ò°¢òòâ—VG&—fRFVÂÆöö·W ¢†7–æ2‚’Óâ°¢–b‚Eô´U’’&WGW&âçVÆÃ°¢G'’°¢6öç7B7"Òv—BDvWB†öFVÇ2÷6V&6ƒ÷FW&ÓÒG¶Væ6öFUU$”6ö×öæVçB‡VW'’—ÒfÆ–Ö—CÓ6’æ6F6‚‚‚’ÓâçVÆÂ“°¢6öç7BFVÇ2Ò7#òæFFòæ—FV×2ÇÂµÓ°¢&WGW&âFVÇ2æÆVæwF‚òFVÇ2¢çVÆÃ°¢Ò6F6‚²&WGW&âçVÆÃ²Ğ¢Ò’‚’À¢òò"âG&÷&÷‚ÖF6‚†6†W&6†RF÷76–W"&÷&œ:—L:’¢†7–æ2‚’Óâ°¢G'’°¢–b‡G—VöbÖF6„G&÷&÷„fæ6RÓÓÒvgVæ7F–öâr’°¢&WGW&âv—BÖF6„G&÷&÷„fæ6R†—46VçG&—2òVW'’¢çVÆÂÂ—46VçG&—2òçVÆÂ¢VW'’“°¢Ğ¢Ò6F6‚·Ğ¢&WGW&âçVÆÃ°¢Ò’‚’À¢òò2â6ö×&&ÆW26VçG&—26’F—7öæ–&ÆR‡6¶—6’2WF‚¢†7–æ2‚’Óâ°¢–b‚&ö6W72æVçbä4TåE$•5õU4U"’&WGW&âçVÆÃ°¢òò2FR67&RÆ÷W&B–6’(	B§W7FR–æfòFR&6P¢&WGW&â²6¶—VC¢t6VçG&—26ö×&&ÆW27W"FVÖæFRW‡Æ–6—FRrÓ°¢Ò’‚’À¢Ó°¢6öç7B¶FVÇ2ÂF'„ÖF6‚Â6VçG&—4–æfõÒÒv—B&öÖ—6RæÆÂ‡F6·2“°¢6öç7BVÆ6VBÒ‚„FFRææ÷r‚’ÒC’ò’çFôf—†VBƒ“° ¢òò6ö×÷6RÆR&÷'@¢6öç7BÆ–æW2Ò¶	ù8¢¤F6†&ö&B&÷&œ:—L:’¢(	BG·VW'—Ò‚G¶VÆ6VG×2–ÂruÓ° ¢òò—VG&—fP¢–b†FVÇ2bbFVÇ2æÆVæwF‚’°¢Æ–æW2çW6‚†¯	øú"—VG&—fR‚G¶FVÇ2æÆVæwF‡ÒFVÂG¶FVÇ2æÆVæwF‚âòw2r¢rwÒ“¢¦“°¢f÷"†6öç7BBöbFVÇ2ç6Æ–6RƒÂ2’’°¢6öç7B—FVÒÒBæ—FVÓ°¢6öç7B7FvRÒ‡G—VöbEõ5DtU2ÓÒwVæFVf–æVBrbbEõ5DtU5¶—FVÒç7FvUö–EÒ’ÇÂ7FvRG¶—FVÒç7FvUö–GÖ°¢Æ–æW2çW6‚†(
